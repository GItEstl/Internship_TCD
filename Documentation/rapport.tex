...
\AxiomC{$op \ : T_1 \times T_2 \rightarrow T$ }
\TrinaryInfC{$\Gamma_t ; \ \Gamma_v ; \vdash e_1 \ op \ e_2 : T $}
\end{prooftree}

\tabto{0cm} {\large \textbf{Unary Operator}}
\begin{prooftree}
\AxiomC{$\Gamma_t ; \ \Gamma_v \vdash e : T $}
\AxiomC{$op \ : T \rightarrow T'$ }
\BinaryInfC{$\Gamma_t ; \ \Gamma_v \vdash op \ e : T' $}
\end{prooftree}
...
\BinaryInfC{$\Gamma_t ; \ \Gamma_v \vdash - \ e : T' $}
\end{prooftree}
...
\BinaryInfC{$\Gamma_t ; \ \Gamma_v \vdash e_1(e_2) \ OK_T $}
\end{prooftree}
...
\begin{prooftree}
\AxiomC{$\Gamma_t ; \ \Gamma_v \vdash e_1 : bool$}
\AxiomC{$\Gamma_t ; \ \Gamma_v \vdash e_2 : void $}
\BinaryInfC{$\Gamma_t ; \ \Gamma_v \vdash while(e_1) \ \{ e_2 \} : void $}
\end{prooftree}

\tabto{0cm} {\large \textbf{Return void}}
\begin{prooftree}
\AxiomC{$ $}
\UnaryInfC{$\Gamma_t ; \ \Gamma_v \vdash return \ OK_{void} $}
\end{prooftree}

\tabto{0cm} {\large \textbf{Return Expression }}
\begin{prooftree}
\AxiomC{$\Gamma_t ; \ \Gamma_v \vdash e : T$}
\UnaryInfC{$\Gamma_t ; \ \Gamma_v \vdash return \ e \ OK_T $}
\end{prooftree}
...
\begin{prooftree}
\AxiomC{$wf_{\Gamma_t}(T)$}
\AxiomC{$wf_{\Gamma_t} (\overrightarrow{T})$}
\AxiomC{$unique(\overrightarrow{y})$}
\AxiomC{$\Gamma_t ; \ \Gamma_v; \ \overrightarrow{x \ \ T} ; \ \overrightarrow{y \ \ T^\prime} \vdash i : OK_T $}
\QuaternaryInfC{$\Gamma_t ; \ \Gamma_v \vdash^d func \ T \ a \  \overrightarrow{x \ \ T} \ \{ \ def \  \overrightarrow{y \ \ T^\prime} \ in \ i \} \ OK_{fd}$}
\end{prooftree}

\tabto{0cm} {\large \textbf{Variable Declaration SUPP}}
\begin{prooftree}
\AxiomC{$\Gamma_t ; \ \Gamma_v \vdash T $}
...
\begin{prooftree}
\AxiomC{$s_1 \vdash e \Downarrow v,s_2$}
\UnaryInfC{$s_1 \vdash a = e \Downarrow s_2[n \mapsto v] $}
\end{prooftree}
...
\begin{prooftree}
\AxiomC{$n \in dom(s)$}
\UnaryInfC{$s \vdash type \ x \Downarrow s::[n \mapsto v_{type}]$}
\end{prooftree}
